...
export const VacancyFilterComponent=(props)=>{
    ...
    const [minSalaryValue, setMinSalaryValue]=useState(props.searchParams.get('salary_gte') || 0)
    const [maxSalaryValue, setMaxSalaryValue]=useState(props.searchParams.get('salary_lte') || SALARY_CEILING)
    
    const calculateLeftPointerPosition=(salaryValue)=>{
        let calcPointerPosition=salaryValue*(extreemRightPointerX-extreemLeftPointerX-FILTER_POINTER_SIZE)/SALARY_CEILING+extreemLeftPointerX
    ...
    const calculateRightPointerPosition=(salaryValue)=>{
        let calcPointerPosition=salaryValue*(extreemRightPointerX-extreemLeftPointerX-FILTER_POINTER_SIZE)/SALARY_CEILING+extreemLeftPointerX+FILTER_POINTER_SIZE
    ...
    const buildFIlterQueryString=()=>{
        ...
        // let queryString=''
        // queryString+=vacancyKeyWords ? 'key_search='+vacancyKeyWords :''
        // queryString+=salaryMin ? 'salary_gte='+salaryMin :''
        // queryString+=salaryMax ? 'salary_lte='+salaryMax :''
        // queryString+=vacancyLocation ? 'location='+vacancyLocation :''
        // queryString+=vacancyResidence ? 'residence_type='+ vacancyResidence :''
        // queryString= queryString ? '?'+ queryString :''

        let queryStringArray=[]
        let queryString=''
        if (vacancyKeyWords) queryStringArray.push('key_search='+vacancyKeyWords);
        if (salaryMin) queryStringArray.push('salary_gte='+salaryMin);
        if (salaryMax) queryStringArray.push('salary_lte='+salaryMax);
        if (vacancyLocation) queryStringArray.push('location='+vacancyLocation);
        if (vacancyResidence) queryStringArray.push('residence_type='+ vacancyResidence);
