...
export const VacancyFilterComponent=(props)=>{
    ...
    const buildFIlterQueryString=()=>{
        ...
        let queryString=''
        if (vacancyKeyWords) queryStringArray.push('key_search='+encodeURIComponent(vacancyKeyWords));
        if (salaryMin) queryStringArray.push('salary_gte='+salaryMin);
        if (salaryMax) queryStringArray.push('salary_lte='+salaryMax);
        if (vacancyLocation) queryStringArray.push('location='+encodeURIComponent(vacancyLocation));
